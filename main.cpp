...
int main(){
    ...
    bool isValid(std::string s) {
        ...
        for (int i = 0; i < s.length(); i++){
			...
			if ( i == closedParentheses && ( ((closedParentheses + openParentheses) % 2) != ((openParentheses+1) % 2)) ) return false;
			// else{ return false; }

			if ( i == closedCurly && ( ((closedCurly + openCurly) % 2) != ((openCurly+1) % 2)) ) return false;
			// else{ return false; }

			if ( i == closedSquare && ( ((closedCurly + openSquare) % 2) != ((openSquare+1) % 2)) ) return false;
			// else{ return false; }
